...
ast_phy2 <- read.tree("data/pruned_trees/astral-mafft_r10_l50_i50_b2_0_pruned/astral_bs10_LPP.tre")

pdf_path <- "figures/tangle_astrals/pruned/astral-mafft_full_vs_pruned_r10_l50_i50_b2_0.pdf"
...
#for pruned trees
ast_phy <- ape::root(ast_phy, "P0203H", edgelabel = T, resolve.root = T)
...
#for pruned trees
ast_phy2 <- ape::root(ast_phy2, "P0203H", edgelabel = T, resolve.root = T)
...
t1 <- ggtree(ast_phy, branch.length = "none") + geom_tiplab(size=1.25, color = as.numeric(as.factor(df3$Order)))
t2 <- ggtree(ast_phy2, branch.length = "none")

d1 <- t1$data

#add some space for tip label in ASTRAL phylo
d1$x<-d1$x+(nchar(d1$label)/5)
...
d2$x <- max(d2$x) - d2$x + max(d1$x) +  max(d1$x)*0.05
