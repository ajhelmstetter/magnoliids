rm(list=ls())

# creates input objects:
limit_perc_length_wanted = 0.25 # for X% of length recovery wanted
limit_perc_nb_wanted = 0.25 # for exons present in X% of the samples
path_to_data = "data/"

####
#all
####
library(tidyverse)
library(ggpubr)
library(ochRe)

#Here, set the file name of the table generated by get_seq_lengths.py
sample.filename = paste0(path_to_data, "test_seq_lengths.txt")

#use their script to import to ensure correct ref lengths
sample.data = as.matrix(read.table(
  sample.filename,
  header = T,
  row.names = 1,
  sep = "\t"
))

#remove X from locus name
colnames(sample.data) <- gsub("X", "", colnames(sample.data))

#view data
#view(sample.data)

#set length of reference exon
reference.len = as.numeric(sample.data[1,])

#extract only sample exon lengths
sample.len = sample.data[2:length(sample.data[, 1]),]

#distribution of lengths
p1 <- data.frame(colnames(sample.data),colMeans(sample.len))
colnames(p1)<-c("exon","mean_length")

summary(p1)
sd(p1$mean_length)

# Basic density plot with mean line and marginal rug
d1 <- ggdensity(
  p1,
  x = "mean_length",
  fill = ochre_pal("healthy_reef")(3)[1],
  color = ochre_pal("healthy_reef")(3)[1],
  alpha = 0.15,
  add = "mean",
  rug = TRUE,
  xlab = "Mean recovered exon length (bp)",
  ylab = "Density"
) + xlim(0,4000) + ylim(0, 0.002)


#distribution of aligned/trimmed supercontig lengths
sup_l<-read.csv("data/r10_auto_b2_0_supercontig_and_exon_post_trim_lengths.csv")

summary(sup_l)
sd(sup_l$exon_length)
sd(sup_l$supercontig_length)

# Basic density plot with mean line and marginal rug
d2 <- ggdensity(
  sup_l,
  x = "exon_length",
  fill = ochre_pal("healthy_reef")(3)[2],
  color = ochre_pal("healthy_reef")(3)[2],
  alpha = 0.15,
  add = "mean",
  rug = TRUE,
  xlab = "Aligned and trimmed exon length (bp)",
  ylab = "Density"
) + xlim(0,4000) + ylim(0, 0.002)

#plots

# Basic density plot with mean line and marginal rug
d3 <- ggdensity(
  sup_l,
  x = "supercontig_length",
  fill = ochre_pal("healthy_reef")(3)[3],
  color = ochre_pal("healthy_reef")(3)[3],
  alpha = 0.15,
  add = "mean",
  rug = TRUE,
  xlab = "Aligned and trimmed supercontig length (bp)",
  ylab = "Density"
) + xlim(0,4000) + ylim(0, 0.002)

#plots
library(patchwork)

( d1 / d2 / d3 ) + plot_annotation(tag_levels = 'a',tag_prefix="(",tag_suffix=")") & theme(plot.tag = element_text(size = 14))

ggsave("figures/figS2_lengths.png",height=12,width=10)

#calculate percentage of exon recovered for each exon in each individual
percent.len = sweep(sample.len, 2, reference.len, "/")
percent.len = ifelse(percent.len > 1, 1, percent.len)

####
# ---- ID low data samples -----
####
percent.len.low = ifelse(percent.len >= 0.50, 1, percent.len)
percent.len.low = ifelse(percent.len < 0.50, 0, percent.len.low)
percent.len.low_df<-data.frame(sort(rowSums(percent.len.low)))
percent.len.low_df$sample<-rownames(percent.len.low_df)
#View(percent.len.low_df)

#species that have <50 loci with 50% or more of exon recovered
low_recovery<- percent.len.low_df[percent.len.low_df$sort.rowSums.percent.len.low..<50,]
colnames(low_recovery)<-c("no_loci","sample")

#write csv
write.csv(low_recovery,"outputs/low_recovery.csv")

#write file than can be run
cat(
paste0("sed -i '/>",low_recovery$sample,"/{N;d;}' *"),
file = "outputs/low_recovery_removal.txt",
sep = "\n"
)

# if percentage exon length recovered is >= X% make value 1
# if not make value 0
percent.len.limit <- percent.len

percent.len.limit[percent.len.limit >= limit_perc_length_wanted] = 1
percent.len.limit[percent.len.limit < limit_perc_length_wanted] = 0

#For each exon calculate number of individuals with >= X% of exon
col <- colSums(percent.len.limit)

#Calculate % individuals with >= X% of exon for each exon
col <- col / (length(sample.data[, 1]) - 1) # -1 for meanlength

#Extract only 75_75 exons from original dataset
length_wanted <- sample.data[, col >= limit_perc_nb_wanted]

#Density plot of exon lengths with rug of actual exon lengths
#make data frame
marker_len <-
  data.frame(names(length_wanted[1,]), as.numeric(length_wanted[1,]))
colnames(marker_len) <- c("locus", "length")

# Basic density plot with mean line and marginal rug
ggdensity(
  marker_len,
  x = "length",
  fill = "#0073C2FF",
  color = "#0073C2FF",
  alpha = 0.15,
  add = "mean",
  rug = TRUE,
  xlab = "Recovered exon length (bp)",
  ylab = "Density"
)

ggsave(paste0("outputs/",(100*limit_perc_length_wanted), "_", (100*limit_perc_nb_wanted), "_exon_length_density.png"))

#Copy list of names into a file, and modify to the format shown in README
cat(
  x = paste0("cp *", marker_len$locus, "_supercontig.FNA* ", (100*limit_perc_length_wanted), "_", (100*limit_perc_nb_wanted),"/"),
  file = paste0("outputs/",(100*limit_perc_length_wanted), "_", (100*limit_perc_nb_wanted), ".txt"),
  sep = "\n"
)

###
# taxonomic scale
###

#read in taxonomy dataset
tn<-read.csv("data/sample_data - samples_for_phylo.csv")

#only those samples output put hybpiper
tn<-tn[tn$included_hybpiper==1,]

#check for differences
setdiff(tn$Namelist,rownames(percent.len))
setdiff(rownames(percent.len),tn$Namelist)

#set thresholds
limits <- c(0.25, 0.5, 0.75, 0.9)

#make empty matrix
exon_stats <- matrix(nrow = length(limits), ncol = length(limits))

#loop through threshold combinations
for (i in 1:length(limits)) {
  for (j in 1:length(limits)) {
    percent.len.limit <- percent.len

    # if percentage exon length recovered is >= 75% make value 1
    # if not make value 0
    percent.len.limit[percent.len.limit >= limits[i]] = 1
    percent.len.limit[percent.len.limit < limits[i]] = 0

    #For each exon calculate number of individuals with >= 75% of exon
    col <- colSums(percent.len.limit != 0)

    #Calculate % individuals with >= 75% of exon for each exon
    col <-
      col / (length(sample.data[, 1]) - 1) # -1 for meanlength

    exon_stats[i, j] <-  table(col >= limits[j])[2]

  }

}

#make table presentable
colnames(exon_stats) <-
  c("25% inds", "50% inds", "75% inds", "90% inds")
rownames(exon_stats) <-
  c("25% exons", "50% exons", "75% exons", "90% exons")
exon_stats[is.na(exon_stats)] <- 0
write.csv(exon_stats, "outputs/exon_filtering_stats.csv")

# if percentage exon length recovered is >= X% make value 1
# if not make value 0
percent.len.limit <- percent.len

percent.len.limit[percent.len.limit >= limit_perc_length_wanted] = 1
percent.len.limit[percent.len.limit < limit_perc_length_wanted] = 0

#For each exon calculate number of individuals with >= X% of exon
col <- colSums(percent.len.limit)

#Calculate % individuals with >= X% of exon for each exon
col <- col / (length(sample.data[, 1]) - 1) # -1 for meanlength

#Extract only 75_75 exons from original dataset
length_wanted <- sample.data[, col >= limit_perc_nb_wanted]

#Density plot of exon lengths with rug of actual exon lengths
#make data frame
marker_len <-
  data.frame(names(length_wanted[1,]), as.numeric(length_wanted[1,]))
colnames(marker_len) <- c("locus", "length")

# Basic density plot with mean line and marginal rug
ggdensity(
  marker_len,
  x = "length",
  fill = "#0073C2FF",
  color = "#0073C2FF",
  alpha = 0.15,
  add = "mean",
  rug = TRUE,
  xlab = "Recovered exon length (bp)",
  ylab = "Density"
)

ggsave(paste0("outputs/",(100*limit_perc_length_wanted), "_", (100*limit_perc_nb_wanted), "_exon_length_density.png"))

#Copy list of names into a file, and modify to the format shown in README
cat(
  x = paste0("cp *", marker_len$locus, "_supercontig.FNA* ", (100*limit_perc_length_wanted), "_", (100*limit_perc_nb_wanted),"/"),
  file = paste0("outputs/",(100*limit_perc_length_wanted), "_", (100*limit_perc_nb_wanted), ".txt"),
  sep = "\n"
)

