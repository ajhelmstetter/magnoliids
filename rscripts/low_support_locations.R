...
#custom node colours based on QS
phy_qs$node.label<-as.numeric(phy_qs$node.label)
phy_qs$node.label[phy_qs$node.label<70]<-0
phy_qs$node.label[phy_qs$node.label>=70]<-100

#custome node colours based on LPP
#custom node colours based on QS
phy$node.label<-as.numeric(phy$node.label)
phy$node.label[phy$node.label<0.9]<-0
phy$node.label[phy$node.label>=0.9]<-1

#
pdf("figures/low_support_locations.pdf",width=10,height=20)
...
legend(
  ...
  col = c(NA, NA, NA, NA, NA, "black", "red"),
  bty = 'n'
)
